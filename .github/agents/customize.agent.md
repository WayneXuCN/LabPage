...
**Files:** `_config.yml`, `_data/{locale}/`, `_pages/{locale}/`, `_posts/`, `_projects/{locale}/`, `_news/{locale}/`, `_books/{locale}/`

This project supports multiple languages. Content and data files are organized by locale subdirectories.
...
**Files:** `_books/{locale}/`, `_layouts/book-review.liquid`, `_layouts/book-shelf.liquid`

- Create book reviews in `_books/{locale}/` directory (e.g., `_books/en_US/book-title.md`)
- Two layout options:
  - `book-review.liquid` – Individual book review page
  - `book-shelf.liquid` – Book collection/shelf display
- Add frontmatter: layout, title, author, date, description, img, etc.
- Supports multilingual book content
...
**Files:** `_projects/{locale}/*.md`

- Create project pages in `_projects/{locale}/` directory
- Add frontmatter: layout, title, description, img, importance
- Support for categories and horizontal/grid display
- Supports multilingual project content

### 11. News/Announcements

**Files:** `_news/{locale}/*.md`

- Add inline announcements or news with links in locale subdirectories
